...
export class MatchingAgent extends EventEmitter {
  ...
  private filterOpportunitiesByPreferences(
    ...
    return opportunities.filter((opp) => {
      ...
      const hasPreferredAsset = preferences.preferences.preferredAssets.some(
        (asset) => opp.assetPair.toLowerCase().includes(asset.toLowerCase()),
      );

      // Check risk tolerance
      const riskMatch = this.isRiskCompatible(
        opp.risk,
        preferences.preferences.riskTolerance,
      );

      // Check capital requirements
      const capitalMatch =
        opp.requiredCapital <= preferences.preferences.maxInvestment;

      // Check minimum return
      const returnMatch =
        opp.expectedReturn >= preferences.preferences.minReturnRate;

      // Check protocol exclusions
      const protocolMatch = !preferences.preferences.excludedProtocols?.some(
        (excluded) =>
  ...
  private isRiskCompatible(oppRisk: string, userRisk: string): boolean {
    ...
    return (
      riskLevels[oppRisk as keyof typeof riskLevels] <=
  ...
  private parseRecommendationsFromAI(
    ...
    let _remainingCapital = maxAllocation;
    ...
    for (const opp of sortedOpportunities.slice(0, 3)) {
      // Max 3 allocations
      if (_remainingCapital <= 0) break;
      ...
      const allocatedAmount = Math.min(
        _remainingCapital * allocationPercentage,
        opp.requiredCapital,
      );

      if (allocatedAmount >= 100) {
        // Minimum $100 allocation
        ...
        _remainingCapital -= allocatedAmount;
      }
    }
  ...
  private calculateOpportunityScore(
    ...
    const riskScore = this.isRiskCompatible(
      opp.risk,
      preferences.preferences.riskTolerance,
    )
      ? 30
  ...
  private calculateAllocationPercentage(
    opp: ArbitrageOpportunity,
    preferences: PreferenceSchema,
    _remainingCapital: number,
  ): number {
    const riskMultipliers = { low: 0.2, medium: 0.4, high: 0.6 };
    const userRiskMultiplier =
      riskMultipliers[preferences.preferences.riskTolerance];
    const oppRiskMultiplier = riskMultipliers[opp.risk];
  ...
  private calculateConfidence(
    ...
    if (
      this.isRiskCompatible(opp.risk, preferences.preferences.riskTolerance)
    ) {
      confidence += 0.2;
    }
  ...
  private generateReasoning(
    ...
    const percentage = (
      (amount / preferences.preferences.maxInvestment) *
      100
    ).toFixed(1);

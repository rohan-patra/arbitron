...
export class AgentOrchestrator extends EventEmitter {
  ...
  private log(level: SystemLog['level'], source: string, message: string, data?: any) {
    ...
    if (this.debugMode) {
      console.log(`[${level.toUpperCase()}] ${source}: ${message}`, data ? data : '');
    }
  ...
  private setupAgentCommunication() {
    this.log('debug', 'orchestrator', 'Setting up agent communication channels');

    // Listen to all agent messages with detailed logging
    this.preferenceAgent.on('message', (message: AgentMessage) => {
      this.log('debug', 'preference-agent', `Message sent: ${message.messageType}`, {
    ...
    this.arbAgent.on('message', (message: AgentMessage) => {
      this.log('debug', 'arbitrage-agent', `Message sent: ${message.messageType}`, {
        recipientId: message.recipientId,
        contentPreview: message.content.substring(0, 100),
      });
      this.handleAgentMessage(message);
    ...
    this.matchingAgent.on('message', (message: AgentMessage) => {
      this.log('debug', 'matching-agent', `Message sent: ${message.messageType}`, {
        recipientId: message.recipientId,
        contentPreview: message.content.substring(0, 100),
      });
      this.handleAgentMessage(message);
    ...
    this.arbAgent.on('opportunityFound', (opportunity: ArbitrageOpportunity) => {
      this.log('info', 'arbitrage-agent', 'New arbitrage opportunity discovered', {
    ...
    this.arbAgent.on('opportunityExpired', (opportunity: ArbitrageOpportunity) => {
    ...
    this.matchingAgent.on('recommendation', (recommendation: AllocationRecommendation) => {
      this.log('info', 'matching-agent', 'New allocation recommendation generated', {
    ...
    this.matchingAgent.on('preferencesUpdated', (schema: PreferenceSchema) => {
      this.log('debug', 'matching-agent', 'User preferences updated in matching engine', {
        userId: schema.userId,
      });
    });

    this.log('info', 'orchestrator', 'Agent communication channels established');
  }
  ...
  private routeMessage(message: AgentMessage) {
    if (!message.recipientId) {
      this.log('debug', 'orchestrator', 'Message has no recipient, broadcasting');
      return;
    }

    this.log('debug', 'orchestrator', `Routing message to ${message.recipientId}`, {
      messageId: message.id,
    ...
    if (message.recipientId === this.preferenceAgent.id) {
      this.log('debug', 'orchestrator', 'Delivering message to preference agent');
      this.preferenceAgent.respondToMessage(message);
    } else if (message.recipientId === this.arbAgent.id) {
      this.log('debug', 'orchestrator', 'Delivering message to arbitrage agent');
      ...
      this.log('warn', 'orchestrator', `Unknown recipient: ${message.recipientId}`);
    }
  }
  ...
  private async triggerMatchingAnalysis() {
    ...
    if (activeOpportunities.length > 0) {
      this.log('info', 'orchestrator', `Starting matching analysis with ${activeOpportunities.length} opportunities`);
      try {
        await this.matchingAgent.analyzeOpportunities(activeOpportunities);
        this.log('info', 'orchestrator', 'Matching analysis completed successfully');
      } catch (error) {
        this.log('error', 'orchestrator', 'Error during matching analysis', error);
      }
    } else {
      this.log('debug', 'orchestrator', 'No active opportunities for matching analysis');
    }
  }
  ...
  async start() {
    ...
    this.log('debug', 'orchestrator', 'Starting arbitrage agent scanning (15s intervals)');
  ...
  async processUserInput(userInput: string, userId: string): Promise<PreferenceSchema> {
    ...
    try {
      const schema = await this.preferenceAgent.processUserPreferences(userInput, userId);
      this.log('info', 'orchestrator', `User preferences processed successfully for ${userId}`);
      return schema;
    } catch (error) {
      this.log('error', 'orchestrator', `Error processing user input for ${userId}`, error);
      throw error;
    }
  }
  ...
  getSystemStatus() {
    const status = {
      ...
      metrics: {
        ...
        totalRecommendations: this.messageHistory.filter(m => m.messageType === 'info' && m.agentType === 'matching').length,
        systemLogs: this.systemLogs.length,
      },
    };

    this.log('debug', 'orchestrator', 'System status requested', status.metrics);
    return status;
  }
  ...
  async simulateAgentConversation(topic: string) {
    ...
    this.log('info', 'orchestrator', `Agent conversation simulation completed for topic: ${topic}`);
  }

  // Enable/disable debug mode
  setDebugMode(enabled: boolean) {
    this.debugMode = enabled;
    this.log('info', 'orchestrator', `Debug mode ${enabled ? 'enabled' : 'disabled'}`);
  }
